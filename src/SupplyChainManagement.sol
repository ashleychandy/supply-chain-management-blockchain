    ...
    return validProducts;
        ...
        function requestReturn(uint256 _productId, string memory _reason) external {
    require(products[_productId].id != 0, "Product does not exist");
    require(
        products[_productId].status == ProductStatus.SentByManufacturer ||
        products[_productId].status == ProductStatus.ReceivedByDistributor ||
        products[_productId].status == ProductStatus.SentByDistributor ||
        products[_productId].status == ProductStatus.ReceivedByRetailer,
        "Product is not in a valid state for return"
    );
    
    _returnRequestCounter++;
    returnRequests[_returnRequestCounter] = ReturnRequest({
        id: _returnRequestCounter,
        productId: _productId,
    ...
    products[_productId].status = ProductStatus.ReturnRequested;
    
    emit ReturnRequested(_returnRequestCounter, _productId, msg.sender, _reason, block.timestamp);
    emit ProductStatusChanged(_productId, oldStatus, ProductStatus.ReturnRequested, block.timestamp);
}

        function processReturn(uint256 _returnRequestId, bool _approved) external onlyOwner {
    ReturnRequest storage request = returnRequests[_returnRequestId];
    require(!request.processed, "Return already processed");
    ...
    if (_approved) {
        ...
        removeProductFromStage(request.productId, product.status);
        addProductToStage(request.productId, ProductStatus.Created);
        emit ProductStatusChanged(request.productId, ProductStatus.ReturnRequested, ProductStatus.Created, block.timestamp);
    } else {
        ProductStatus previousStatus = getPreviousStatus(product.status);
        product.status = previousStatus;
        emit ProductStatusChanged(request.productId, ProductStatus.ReturnRequested, previousStatus, block.timestamp);
    }
    
    emit ReturnProcessed(_returnRequestId, _approved, block.timestamp);
}

function getPreviousStatus(ProductStatus _currentStatus) internal pure returns (ProductStatus) {
    if (_currentStatus == ProductStatus.SentByManufacturer) return ProductStatus.Created;
    if (_currentStatus == ProductStatus.ReceivedByDistributor) return ProductStatus.SentByManufacturer;
    if (_currentStatus == ProductStatus.SentByDistributor) return ProductStatus.ReceivedByDistributor;
    if (_currentStatus == ProductStatus.ReceivedByRetailer) return ProductStatus.SentByDistributor;
    return ProductStatus.Created;
}

    function getReturnRequestsForProduct(uint256 _productId) external view returns (ReturnRequest[] memory) {
    uint256 requestCount = 0;
    for (uint256 i = 1; i <= _returnRequestCounter; i++) {
        if (returnRequests[i].productId == _productId) {
            requestCount++;
        }
    }

    ReturnRequest[] memory productRequests = new ReturnRequest[](requestCount);
    uint256 index = 0;
    for (uint256 i = 1; i <= _returnRequestCounter; i++) {
        if (returnRequests[i].productId == _productId) {
            productRequests[index] = returnRequests[i];
            index++;
        }
    }
    ...
    return productRequests;
        ...
        function getPendingReturnRequests() external view returns (ReturnRequest[] memory) {
    uint256 pendingCount = 0;
    for (uint256 i = 1; i <= _returnRequestCounter; i++) {
        if (!returnRequests[i].processed) {
            pendingCount++;
        }
    }

    ReturnRequest[] memory pendingRequests = new ReturnRequest[](pendingCount);
    uint256 index = 0;
    for (uint256 i = 1; i <= _returnRequestCounter; i++) {
        if (!returnRequests[i].processed) {
            pendingRequests[index] = returnRequests[i];
            index++;
        }
    }
